import Tesseract from 'tesseract.js';
import sharp from 'sharp';

interface ToonResult {
    type: 'text' | 'toon';
    content: string;
}

/**
 * Extract text from image using OCR (Optimized Preprocessing, Low RAM)
 * @param {Buffer} imageBuffer - Image buffer
 * @returns {Promise<string>} - Extracted text
 */
export async function extractTextFromImage(imageBuffer: Buffer): Promise<string> {
    let worker: Tesseract.Worker | null = null;
    try {
        const start = Date.now();
        console.log('[OCR] 🖼️  Preprocessing image (Resize + Grayscale)...');

        // Preprocess with sharp for better OCR accuracy
        const processedBuffer = await sharp(imageBuffer)
            .resize({ width: 2500, withoutEnlargement: true }) // Ensure good resolution for text
            .grayscale() // Remove color noise
            .normalize() // Improve contrast
            .toBuffer();

        console.log('[OCR] 🚀 Starting Tesseract Worker...');
        worker = await Tesseract.createWorker('eng');

        console.log('[OCR] 📖 Recognizing text...');
        // @ts-ignore - Tesseract types might vary slightly, treating as any for safety
        const result = await worker.recognize(processedBuffer);
        const text = result.data.text;

        const duration = ((Date.now() - start) / 1000).toFixed(2);
        console.log(`[OCR] ✅ Extracted ${text.length} chars in ${duration}s`);
        return text;

    } catch (error: any) {
        console.error('[OCR] Failed:', error.message);
        throw error;
    } finally {
        if (worker) {
            console.log('[OCR] 🧹 Terminating worker to free RAM...');
            await worker.terminate();
        }
    }
}

/**
 * Detect table structure in OCR text
 * @param {string} text - OCR extracted text
 * @returns {Array} - Table rows
 */
export function detectTableInOCR(text: string): string[][] {
    const lines = text.split('\n').filter(l => l.trim());
    const table: string[][] = [];

    for (const line of lines) {
        // Split by multiple spaces or tabs
        const parts = line.split(/\s{2,}|\t/).filter(p => p.trim());

        if (parts.length >= 3) {
            table.push(parts);
        }
    }

    return table;
}

/**
 * Convert image table to TOON
 * @param {Array} table - Table rows
 * @returns {string} - TOON string
 */
export function imageTableToToon(table: string[][]): string {
    if (!table || table.length < 2) return '';

    const headers = table[0];
    const rows = table.slice(1);

    let toon = `@SCHEMA|${headers.join('|')}\n`;

    rows.forEach(row => {
        const paddedRow = [...row];
        while (paddedRow.length < headers.length) {
            paddedRow.push('');
        }
        toon += paddedRow.slice(0, headers.length).join('|') + '\n';
    });

    return toon;
}

/**
 * Main function: Image → TOON (no LLM!)
 * @param {Buffer} imageBuffer - Image buffer
 * @returns {Promise<Object>} - TOON data or text
 */
export async function processImageToToon(imageBuffer: Buffer): Promise<ToonResult> {
    console.log('[Image→TOON] Extracting text with OCR...');

    const text = await extractTextFromImage(imageBuffer);

    console.log('[Image→TOON] Detecting table structure...');
    const table = detectTableInOCR(text);

    if (table.length < 2) {
        console.log('[Image→TOON] No table detected, returning raw text');
        return { type: 'text', content: text };
    }

    console.log(`[Image→TOON] Found table with ${table.length} rows`);

    const toonString = imageTableToToon(table);

    console.log(`[Image→TOON] ✅ Converted to TOON`);

    return { type: 'toon', content: toonString };
}
