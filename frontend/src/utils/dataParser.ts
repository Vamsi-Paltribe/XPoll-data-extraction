 ...
 */
export async function parseImageToJSON(file: File, _onProgress?: (progress: number) => void): Promise<ParserResult> {
    try {
        ...
        const pageData = data as any; // Cast to any to access potentially missing specific props in Types
 ...
 */
function parseTextToJSON(text: string): { success: boolean, data?: any[], delimiter?: string, method?: string } {
 ...
 */
function parseSpaceSeparated(lines: string[]): { success: boolean, data?: any[], delimiter?: string, method?: string } {
