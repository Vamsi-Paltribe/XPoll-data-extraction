 ...
 */
export async function parseImageToJSON(file: File, onProgress?: (progress: number) => void): Promise<ParserResult> {
    try {
 ...
 */
export async function parseExcelToJSON(file: File): Promise<ParserResult> {
    try {
 ...
 */
export async function parsePDFToJSON(file: File): Promise<ParserResult> {
    let fullText = "";
    ...
    try {
        ...
        const pagesData: { page: number; content: string }[] = [];
 ...
 */
export async function parseTXTToJSON(file: File): Promise<ParserResult> {
    try {
 ...
 */
function parseTextToJSON(text: string): { success: boolean; data?: any[]; delimiter?: string; parseMethod?: string } {
    ...
    for (const delimiter of delimiters) {
        try {
            const parseResult = Papa.parse(text, {
                delimiter: delimiter.char instanceof RegExp ? '' : delimiter.char as string,
                header: true,
                skipEmptyLines: true,
                dynamicTyping: false,
 ...
 */
function parseSpaceSeparated(lines: string[]): { success: boolean; data?: any[]; delimiter?: string } {
 ...
 */
export function createSmartSample(data: any[], sampleSize = 100) {
 ...
 */
export function deduplicateData(data: any[]) {
    if (!Array.isArray(data) || data.length === 0) {
...
export function analyzeDataQuality(data: any[]) {
    ...
    let emptyFields = 0;
    let fieldTypes: Record<string, { empty: number; filled: number; types: Set<string> }> = {};
